...
static void update_switch_event_handler(struct _lv_event_t *e)
{
    ...
    if (strcmp(current_text, "检查更新") == 0)
    {
        lv_obj_set_style_bg_color(update_switch, lv_color_hex(0x90EE90),
                                  LV_PART_MAIN | LV_STATE_DEFAULT);
        // 调用OTA检查版本函数
        int result = dfu_pan_query_versions(
            OTA_QUERY_URL, VERSION, latest_version, sizeof(latest_version));
        // 根据返回值判断是否有更新
...
static void update_confirm_button_event_handler(lv_event_t *e)
{
    ...
    if (code == LV_EVENT_CLICKED)
    {
        ...
        if (strcmp(button_text, "更新") == 0)
        {
            ...
            // 在这里设置更新标志位
            if (dfu_pan_mark_versions_for_update() != 0)
            {
...
#define CONT_H_PER(x) ((CONT_H) * (x) / 100)
    ...
    lv_obj_align_to(charging_icon, battery_outline, LV_ALIGN_OUT_LEFT_MID, 0,
                    0);
...
static void pm_event_handler(gui_pm_event_type_t event)
{
    ...
    case GUI_PM_EVT_RESUME:
    {
        ...
        if (shutdown_state &&
            lowpower_shutdown_state) // 如果是关机消息触发的唤醒，就不再切换到对话界面去了
        {
            rt_kprintf("恢复屏幕-> 对话\n");
    ...
    while (1)
    {
        ...
        while (rt_mq_recv(ui_msg_queue, &msg, sizeof(ui_msg_t *), 0) == RT_EOK)
        {
            ...
            case UI_MSG_UPDATE_LATEST_VERSION:
                ...
                break;
            case UI_MSG_CONFIRM_BUTTON_EVENT: // 如果使用了新的消息类型，则改为对应的新类型
            {
                // 从消息数据中获取按钮类型
        ...
        if (RT_EOK == rt_sem_trytake(&update_ui_sema))
        {
            ...
            if (lv_display_get_inactive_time(NULL) > IDLE_TIME_LIMIT &&
                ...
                g_pan_connected) // 如果当前满足了屏幕不活跃的时间，并且当前屏幕不是待机屏幕，当前屏幕不是开机启动屏幕，当前屏幕不是关机屏幕，当前屏幕不是睡眠屏幕
