...
{
    if (anim_tick && (anim_tick + ANIM_TIMEOUT < lv_tick_get()) &&
        0 == (cont_status & CONT_IDLE))
    {
...
static lv_obj_t *create_tip_label(lv_obj_t *parent, const char *tips,
...
static lv_obj_t *create_switch(lv_obj_t *parent, lv_event_cb_t cb, uint8_t row,
                               uint8_t col, uint8_t checked)
{
...
static lv_obj_t *create_slider(lv_obj_t *parent, lv_event_cb_t cb, uint8_t row,
                               uint8_t col, int32_t min, int32_t max,
                               uint8_t val)
{
...
static lv_obj_t *create_lines(lv_obj_t *parent, lv_event_cb_t cb, uint8_t row,
                              uint8_t col, uint16_t cnt, uint16_t val)
{
    ...
    for (uint32_t i = 0; i < cnt; i++)
    {
        ...
        if (brigtness_tb[i] <= val)
            lv_obj_set_style_bg_color(
                btn, lv_palette_main(LV_PALETTE_LIGHT_GREEN), 0);
        else
...
static lv_obj_t *create_button(lv_obj_t *parent, lv_event_cb_t cb,
                               const char *text, uint8_t row, uint8_t col)
{
    ...
    // 设置默认按钮颜色
    lv_obj_set_style_bg_color(btn, lv_color_hex(0x1976D2),
...
static void slider_event_handler(struct _lv_event_t *e)
{
    ...
    audio_server_set_private_volume(AUDIO_TYPE_LOCAL_MUSIC,
                                    lv_slider_get_value(slider)); // 设置音量
}
...
static void line_event_handler(struct _lv_event_t *e)
{
    ...
    while (i < cnt)
    {
        ...
        {
            lv_obj_set_style_bg_color(
                child, lv_palette_main(LV_PALETTE_LIGHT_GREEN), 0);
        }
        else
        {
            lv_obj_set_style_bg_color(child, lv_palette_main(LV_PALETTE_GREY),
                                      0);
        }
        i++;
    }
...
static void update_switch_event_handler(struct _lv_event_t *e)
{
    ...
    {
        lv_obj_set_style_bg_color(update_switch, lv_color_hex(0x90EE90),
                                  LV_PART_MAIN | LV_STATE_DEFAULT);
        // 调用OTA检查版本函数
        // 构建动态URL
        ...
        xiaozhi_ui_update_confirm_popup(UI_MSG_SHOW_UPDATE_CONFIRM,
                                        needs_update);
    }
}
// 添加弹框按钮事件处理函数
static void update_confirm_button_event_handler(lv_event_t *e)
{
    ...
    if (code == LV_EVENT_CLICKED)
    {
        ...
        else if (strcmp(button_text, "取消") == 0)
        {
            ...
            // 将下滑菜单按钮恢复颜色
            lv_obj_set_style_bg_color(update_switch, lv_color_hex(0x1976D2),
                                      LV_PART_MAIN);
...
rt_err_t xiaozhi_ui_obj_init()
{
    ...
    lv_obj_set_style_arc_color(battery_arc, lv_color_hex(0x333333),
                               LV_PART_MAIN); // 背景圆环颜色
    lv_obj_set_style_arc_color(battery_arc, lv_color_hex(0x00CC00),
                               LV_PART_INDICATOR); // 电量颜色(绿色)
    lv_obj_set_style_arc_width(battery_arc, (int)(8 * g_scale),
                               LV_PART_MAIN); // 背景圆环宽度
    lv_obj_set_style_arc_width(battery_arc, (int)(6 * g_scale),
                               LV_PART_INDICATOR); // 电量指示宽度
    // 电池电量百分比文本
    ...
    lv_obj_align(standby_charging_icon, LV_ALIGN_CENTER, 0,
                 0); // 在圆弧中心对齐
...
#if USING_TOUCH_SWITCH
    lv_obj_add_event_cb(header_row, header_row_event_handler, LV_EVENT_ALL,
                        NULL);
#endif
...
#define CONT_H_PER(x) ((CONT_H) * (x) / 100)
    ...
    volume_slider =
        create_slider(cont, slider_event_handler, 3, 1, VOL_MIN_LEVEL,
                      VOL_MAX_LEVEL, VOL_DEFAULE_LEVEL);
    create_tip_label(cont, "亮度", 4, 0);
    brightness_lines = create_lines(cont, line_event_handler, 4, 1, BRT_TB_SIZE,
                                    LCD_BRIGHTNESS_DEFAULT);
    create_tip_label(cont, "代理", 5, 0);
    handsfree_switch =
        create_switch(cont, hfp_handsfree_switch_event_handler, 5, 1, 0);
    update_switch =
        create_button(cont, update_switch_event_handler, "检查更新", 5, 2);
    ...
    lv_obj_set_style_border_color(update_confirm_popup, lv_color_hex(0x00a0ff),
                                  0);
    ...
    lv_obj_set_style_text_color(update_confirm_label, lv_color_hex(0xFFFFFF),
                                0);
    ...
    lv_obj_add_event_cb(update_button, update_confirm_button_event_handler,
    ...
    lv_obj_add_event_cb(cancel_button, update_confirm_button_event_handler,
                        LV_EVENT_CLICKED, NULL);
    ...
    lv_obj_set_style_bg_color(g_battery_fill, lv_color_hex(0x00ff00),
                              0); // 初始绿色
    ...
    lv_obj_align(charging_icon, LV_ALIGN_RIGHT_MID, 8,
                 0); // 电池框左侧，稍微有点间距
...
static void pm_event_handler(gui_pm_event_type_t event)
{
    ...
    case GUI_PM_EVT_RESUME:
    {
        ...
        if (shutdown_state && lowpower_shutdown_state &&
            !s_talk_with_hfp) // 如果是关机消息 和 通话界面按键
                              // 触发的唤醒，就不再切换到对话界面去了
        {
            rt_kprintf("恢复屏幕-> 对话\n");
...
void xiaozhi_update_battery_level(int level)
{
    ...
    if (battery_arc)
    {
        ...
        {
            // 低电量红色
            lv_obj_set_style_arc_color(battery_arc, lv_color_hex(0xCC0000),
                                       LV_PART_INDICATOR);
        }
        else if (g_battery_level <= 50)
        {
            // 中等电量黄色
            lv_obj_set_style_arc_color(battery_arc, lv_color_hex(0xCCCC00),
                                       LV_PART_INDICATOR);
        }
        else
        {
...
void xiaozhi_ui_task(void *args)
{
    ...
    // 初始化UI消息队列
    ui_msg_queue =
        rt_mq_create("ui_msg", sizeof(ui_msg_t *), 20, RT_IPC_FLAG_FIFO);
    if (ui_msg_queue == RT_NULL)
    {
    ...
    if (!g_skip_startup)
    {
        ...
        lv_font_t *font = lv_tiny_ttf_create_file("A:/DroidSansFallback.ttf",
                                                  adjusted_font_size);
#else
        lv_font_t *font = lv_tiny_ttf_create_data(
            xiaozhi_font, xiaozhi_font_size, adjusted_font_size);
#endif
...
#if defined(TINY_TTF_MODE)
    font_medium =
        lv_tiny_ttf_create_file("A:/DroidSansFallback.ttf", medium_font_size);
#else
    font_medium = lv_tiny_ttf_create_data(xiaozhi_font, xiaozhi_font_size,
                                          medium_font_size);
#endif
...
#if defined(TINY_TTF_MODE)
    lv_font_t *font =
        lv_tiny_ttf_create_file("A:/DroidSansFallback.ttf", adjusted_font_size);
#else
    ...
    if (!update_time_ui_timer)
    {
        update_time_ui_timer =
            rt_timer_create("update_ui_time", update_xiaozhi_ui_time, NULL,
    ...
    if (!update_weather_ui_timer)
    {
        update_weather_ui_timer =
            rt_timer_create("update_ui_time", update_xiaozhi_ui_weather, NULL,
    ...
    while (1)
    {
        ...
        while (rt_mq_recv(ui_msg_queue, &msg, sizeof(ui_msg_t *), 0) == RT_EOK)
        {
                ...
                if (msg->data)
                {
                    if (strcmp(msg->data, "sleepy") == 0)
                    {
                        if (img_emoji)
                        {
                            ...
                            lv_img_set_src(
                                img_emoji,
                                "A:/sleepy2.ezip"); // 使用睡眠表情表示小智未连接
#else
                            // 使用传统C数组模式
                            lv_img_set_src(
                                img_emoji,
                                &sleepy2); // 使用睡眠表情表示小智未连接
#endif
                        }
                    }
                    else if (strcmp(msg->data, "funny") == 0)
                    {
                        if (img_emoji)
                        {
                            ...
                            lv_img_set_src(
                                img_emoji,
                                "A:/funny2.ezip"); // 使用睡眠表情表示小智未连接
#else
                            // 使用传统C数组模式
                            lv_img_set_src(
                                img_emoji,
                                &funny2); // 使用睡眠表情表示小智未连接
#endif
                        }
                    }
                }
            ...
            case UI_MSG_SWITCH_TO_STANDBY:
                ...
                if (g_pan_connected)
                {
                    ...
                    ui_sleep_timer =
                        lv_timer_create(ui_sleep_callback, 60000, NULL);
                }
                ...
                // 创建定时器，稍后执行更新
                standby_update_timer =
                    lv_timer_create(standby_update_callback, 100, NULL);
                    ...
                    if (brightness_lines)
                    {
                        ...
                        while (i < cnt)
                        {
                            ...
                            if (brigtness_tb[i] <= brightness)
                                ...
                                lv_obj_set_style_bg_color(
                                    child, lv_palette_main(LV_PALETTE_GREY), 0);
                            i++;
                        }
                    }
                }
            ...
            case UI_MSG_CHARGE_STATUS_CHANGED:
                if (msg->data)
                {
                    ...
                    bool should_show_charging =
                        is_charging && (g_battery_level < 100);
                    if (charging_icon)
                    {
                        if (should_show_charging)
                        {
                            lv_obj_clear_flag(charging_icon,
                                              LV_OBJ_FLAG_HIDDEN);
                            rt_kprintf("显示充电图标\n");
                        }
                        else
                    ...
                    if (standby_charging_icon)
                    {
                        if (should_show_charging)
                        {
                            lv_obj_clear_flag(standby_charging_icon,
                                              LV_OBJ_FLAG_HIDDEN);
                        ...
                        {
                            lv_obj_add_flag(standby_charging_icon,
                                            LV_OBJ_FLAG_HIDDEN);
                            if (is_charging && g_battery_level >= 100)
                            {
                                rt_kprintf("电量已满，隐藏待机界面充电图标 "
                                           "(电量: %d%%)\n",
                                           g_battery_level);
                            }
                            else
            ...
            case UI_MSG_UPDATE_EMOJI:
                if (msg->data)
                {
                    ...
                    {
                        lv_seqimg_file_array(seqimg, neutral_files,
                                             sizeof(neutral_files) /
                                                 sizeof(neutral_files[0]));
                    }
                    else if (strcmp(msg->data, "happy") == 0)
                    {
                        lv_seqimg_file_array(seqimg, funny_files,
                                             sizeof(funny_files) /
                                                 sizeof(funny_files[0]));
                    }
                    ...
                    {
                        lv_seqimg_file_array(seqimg, crying_files,
                                             sizeof(crying_files) /
                                                 sizeof(crying_files[0]));
                    }
                    else if (strcmp(msg->data, "angry") == 0)
                    {
                        lv_seqimg_file_array(seqimg, angry_files,
                                             sizeof(angry_files) /
                                                 sizeof(angry_files[0]));
                    }
                    ...
                    {
                        lv_seqimg_file_array(seqimg, loving_files,
                                             sizeof(loving_files) /
                                                 sizeof(loving_files[0]));
                    }
                    else if (strcmp(msg->data, "embarrassed") == 0)
                    {
                        lv_seqimg_file_array(seqimg, embarrassed_files,
                                             sizeof(embarrassed_files) /
                                                 sizeof(embarrassed_files[0]));
                    }
                    else if (strcmp(msg->data, "surprised") == 0)
                    {
                        lv_seqimg_file_array(seqimg, surprised_files,
                                             sizeof(surprised_files) /
                                                 sizeof(surprised_files[0]));
                    }
                    ...
                    {
                        lv_seqimg_file_array(seqimg, thinking_files,
                                             sizeof(thinking_files) /
                                                 sizeof(thinking_files[0]));
                    }
                    ...
                    {
                        lv_seqimg_file_array(seqimg, cool_files,
                                             sizeof(cool_files) /
                                                 sizeof(cool_files[0]));
                    }
                    ...
                    {
                        lv_seqimg_file_array(seqimg, kissy_files,
                                             sizeof(kissy_files) /
                                                 sizeof(kissy_files[0]));
                    }
                    ...
                    {
                        lv_seqimg_file_array(seqimg, sleepy_files,
                                             sizeof(sleepy_files) /
                                                 sizeof(sleepy_files[0]));
                    }
                    ...
                    {
                        lv_seqimg_file_array(
                            seqimg, neutral_files,
                            sizeof(neutral_files) /
                                sizeof(neutral_files[0])); // common emoji is
                                                           // neutral
                    }
#else
                    ...
                    if (strcmp(msg->data, "neutral") == 0)
                    {
                        lv_seqimg_src_array(seqimg, neutral,
                                            sizeof(neutral) /
                                                sizeof(neutral[0]));
                    }
                    else if (strcmp(msg->data, "happy") == 0)
                    {
                        lv_seqimg_src_array(seqimg, funny,
                                            sizeof(funny) / sizeof(funny[0]));
                    }
                    else if (strcmp(msg->data, "laughing") == 0)
                    {
                        lv_seqimg_src_array(seqimg, funny,
                    ...
                    {
                        lv_seqimg_src_array(seqimg, crying,
                                            sizeof(crying) / sizeof(crying[0]));
                    }
                    else if (strcmp(msg->data, "angry") == 0)
                    {
                        lv_seqimg_src_array(seqimg, angry,
                                            sizeof(angry) / sizeof(angry[0]));
                    }
                    else if (strcmp(msg->data, "crying") == 0)
                    {
                        lv_seqimg_src_array(seqimg, crying,
                    ...
                    {
                        lv_seqimg_src_array(seqimg, loving,
                                            sizeof(loving) / sizeof(loving[0]));
                    }
                    else if (strcmp(msg->data, "embarrassed") == 0)
                    {
                        lv_seqimg_src_array(seqimg, embarrassed,
                                            sizeof(embarrassed) /
                                                sizeof(embarrassed[0]));
                    }
                    else if (strcmp(msg->data, "surprised") == 0)
                    {
                        lv_seqimg_src_array(seqimg, surprised,
                                            sizeof(surprised) /
                                                sizeof(surprised[0]));
                    }
                    else if (strcmp(msg->data, "shocked") == 0)
                    ...
                    {
                        lv_seqimg_src_array(seqimg, thinking,
                                            sizeof(thinking) /
                                                sizeof(thinking[0]));
                    }
                    else if (strcmp(msg->data, "winking") == 0)
                    ...
                    {
                        lv_seqimg_src_array(seqimg, cool,
                                            sizeof(cool) / sizeof(cool[0]));
                    }
                    else if (strcmp(msg->data, "relaxed") == 0)
                    {
                        lv_seqimg_src_array(seqimg, cool,
                    ...
                    {
                        lv_seqimg_src_array(seqimg, kissy,
                                            sizeof(kissy) / sizeof(kissy[0]));
                    }
                    else if (strcmp(msg->data, "confident") == 0)
                    {
                        lv_seqimg_src_array(seqimg, cool,
                    ...
                    {
                        lv_seqimg_src_array(seqimg, sleepy,
                                            sizeof(sleepy) / sizeof(sleepy[0]));
                    }
                    else if (strcmp(msg->data, "silly") == 0)
                    {
                        lv_seqimg_src_array(seqimg, thinking,
                    ...
                    else
                    {
                        lv_seqimg_src_array(
                            seqimg, neutral,
                            sizeof(neutral) /
                                sizeof(neutral[0])); // common emoji is neutral
                    }
#endif
                }
                break;
            ...
            case UI_MSG_TTS_OUTPUT:
                if (msg->data)
                {
                    ...
                    if (len > SHOW_TEXT_LEN)
                    {
                        ...
                        // 向前调整到完整的 UTF-8 字符起点
                        while (cut_pos > 0 &&
                               ((unsigned char)msg->data[cut_pos] & 0xC0) ==
                                   0x80)
                        {
                            cut_pos--;
                        }
                        ...
                        // 剩余部分从 cut_pos 开始
                        strncpy(g_second_part, msg->data + cut_pos,
                                sizeof(g_second_part) - 1);
                        g_second_part[sizeof(g_second_part) - 1] =
                            '\0'; // 确保结尾
            ...
            case UI_MSG_TTS_SWITCH_PART:
                if (g_label_for_second_part && strlen(g_second_part) > 0)
                {
                    ...
                    if (len > SHOW_TEXT_LEN)
                    {
                        ...
                        int cut_pos = SHOW_TEXT_LEN;
                        while (cut_pos > 0 &&
                               ((unsigned char)g_second_part[cut_pos] & 0xC0) ==
                                   0x80)
                        {
                            cut_pos--;
                        }
                        ...
                        strncpy(remaining, g_second_part + cut_pos,
                                sizeof(remaining) - 1);
                        ...
                        // 保存剩余部分
                        strncpy(g_second_part, remaining,
                                sizeof(g_second_part) - 1);
                        g_second_part[sizeof(g_second_part) - 1] = '\0';

                        // 重置定时器以显示下一部分
                        rt_timer_control(
                            g_split_text_timer, RT_TIMER_CTRL_SET_TIME,
                    ...
                    {
                        // 最后一部分，直接显示
                        lv_label_set_text(g_label_for_second_part,
                                          g_second_part);
            ...
            case UI_MSG_CONFIRM_BUTTON_EVENT:
                ...
                // 检查弹框是否已创建且可见
                if (update_confirm_popup &&
                    !lv_obj_has_flag(update_confirm_popup, LV_OBJ_FLAG_HIDDEN))
                {
                    if (is_update_button)
                    {
                        // 模拟点击更新按钮
                        ...
                        {
                            lv_obj_send_event(update_button, LV_EVENT_CLICKED,
                                              NULL);
                        }
                    }
                    else
        ...
        if (RT_EOK == rt_sem_trytake(&update_ui_sema))
        {
            ...
            // current_screen, main_container); rt_kprintf("inactive_time: %d,
            // limit: %d\n", lv_display_get_inactive_time(NULL),
            // IDLE_TIME_LIMIT);
            if (lv_display_get_inactive_time(NULL) > IDLE_TIME_LIMIT &&
                current_screen != standby_screen &&
                current_screen != g_startup_screen &&
                current_screen != shutdown_screen &&
                current_screen != sleep_screen &&
                current_screen != low_battery_shutdown_screen &&
                g_pan_connected &&
                current_screen !=
                    call_screen) // 如果当前满足了屏幕不活跃的时间，并且当前屏幕不是待机屏幕，当前屏幕不是开机启动屏幕，当前屏幕不是关机屏幕，当前屏幕不是睡眠屏幕
            ...
            if (mqtt_g_state == kDeviceStateUnknown && last_listen_tick > 0 &&
                g_pan_connected && she_bei_ma && !s_talk_with_hfp)
#else
                ...
                if (delta < rt_tick_from_millisecond(12000))
                {
                    ...
                    bt_interface_wr_link_policy_setting(
                        ...
                        BT_NOTIFY_LINK_POLICY_SNIFF_MODE |
                            BT_NOTIFY_LINK_POLICY_ROLE_SWITCH); // open role
                                                                // switch
                    MCP_RGBLED_CLOSE();
                    show_sleep_countdown_and_sleep();
