...
void show_sleep_countdown_and_sleep(void)
{
    ...
    if (!g_tip_font)
        g_tip_font = lv_tiny_ttf_create_data(xiaozhi_font, xiaozhi_font_size,
                                             tip_font_size);
    if (!g_big_font)
        g_big_font = lv_tiny_ttf_create_data(xiaozhi_font, xiaozhi_font_size,
                                             big_font_size);
...
{
    if (g_low_battery_shutdown_active)
        return;                        // 已经在倒计时，直接返回
    ...
    if (!low_battery_shutdown_screen)
    {
        ...
        lv_obj_set_style_bg_color(low_battery_shutdown_screen,
    ...
    lv_style_set_text_color(&style_tip_low_battery,
    ...
    lv_style_set_text_color(&style_big_low_battery,
                            lv_color_hex(0xFF0000)); // 红色
    ...
    if (low_battery_shutdown_timer)
        lv_timer_delete(low_battery_shutdown_timer);
    low_battery_shutdown_timer =
        lv_timer_create(low_battery_shutdown_countdown_cb, 1000, NULL);
...
{
    if (g_shutdown_countdown_active)
        return;                      // 已经在倒计时，直接返回
...
{
    if (g_low_battery_warning_active)
        return;
    ...
    if (!low_battery_warning_screen)
    {
        ...
        lv_obj_set_style_bg_color(low_battery_warning_screen,
                                  lv_color_hex(0x000000), 0);
    }
    ...
    warning_shutdown_timer = lv_timer_create(
        low_battery_warning_simple_timer_cb, 5000, NULL); // 直接5秒后执行
...
static void startup_fadeout_ready_cb(struct _lv_anim_t *anim)
{
    ...
    if (standby_screen && now_screen != low_battery_shutdown_screen &&
        now_screen != low_battery_warning_screen &&
        now_screen != shutdown_screen && now_screen != sleep_screen)
    {
        rt_kprintf("开机->待机");
...
static void startup_anim_ready_cb(struct _lv_anim_t *anim)
{
    ...
    lv_timer_t *fadeout_timer =
        lv_timer_create(startup_fadeout_timer_cb, 1500, NULL);
...
void show_startup_animation(void)
{
    ...
    lv_obj_set_size(g_startup_screen, lv_disp_get_hor_res(NULL),
                    lv_disp_get_ver_res(NULL));
    lv_obj_set_style_bg_color(g_startup_screen, lv_color_hex(0x000000),
                              0); // 黑色背景
    ...
    lv_obj_set_size(g_startup_img, SCALE_DPX(180),
                    SCALE_DPX(92)); // 宽180dp，高92dp
    lv_img_set_zoom(g_startup_img,
